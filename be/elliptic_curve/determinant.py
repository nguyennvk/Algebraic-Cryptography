from flask import request, jsonify, Blueprint
from sympy import isprime

determinant = Blueprint('determinant', __name__)
@determinant.route('/elliptic_curve/determinant', methods=['GET'])
def get_determinant():
    """
    Calculate the determinant of an elliptic curve given the coefficients a, b, and p.
    """
    # Sanitize input
    a = request.args.get('a', type=int)
    b = request.args.get('b', type=int)
    p = request.args.get('p', type=int)
    if a is None or b is None or p is None:
        return jsonify({"error": "Please provide 'a', 'b', and 'p' parameters."}), 400
    if p <= 0:
        return jsonify({"error": "Please provide a positive integer for 'p'."}), 400
    if not isprime(p):
        return jsonify({"error": "p must be a prime number."}), 400

    # Calculate the discriminant
    discriminant = (4 * a**3 + 27 * b**2) % p

    # Check if the discriminant is zero
    if discriminant == 0:
        return jsonify({"determinant": 0, "message": "The elliptic curve is singular."}), 200
    else:
        return jsonify({"determinant": discriminant, "message": "The elliptic curve is non-singular."}), 200