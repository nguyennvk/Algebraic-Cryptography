...
export default function CarmichaelFuncPage() {
    ...
    const [solution, setSolution] = useState({"points": []});
    ...
    return (
        <div>
            <div className="flex flex-col items-center">
                <h1 className="text-4xl">Elliptic Curve points</h1>
                <p className="text-lg mt-4">
                    Return all the points on an Elliptic Curve <span>{<LatexText expression="y^2=x^3+ax+b"></LatexText>}</span> lives 
                ...
                <div className="mt-5" style={{display: equation ? "block" : "none"}}>
                    <SolveButton onClick={() => {
                            fetch(`${BACKEND_URL}/elliptic_curve/points?a=${a}&b=${b}&p=${p}`)
                            .then(async (res) => {
                                if (!res.ok) {
