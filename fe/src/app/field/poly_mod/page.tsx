    ...
    return (
        <div>
            <div className="flex flex-col items-center">
                <h1 className="text-4xl">Modulo division of polynomial function</h1>
                <p className="text-lg mt-4">
                    Return remainder of when divide 2 polynomial functions.
                </p>
                <div>
                    <SimpleInput value={poly1} name="Dividend" onChange={handlePoly1Change}/>
                    <SimpleInput value={poly2} name="Divisor" onChange={handlePoly2Change}/>
                    <SimpleInput value={p} name="p" onChange={handlePChange}/>
                    <p className="text-lg mt-4">Dividend: <span>{<LatexText expression={`${poly1}`}></LatexText>}</span></p>
                    <p className="text-lg mt-4">Divisor: <span>{<LatexText expression={`${poly2}`}></LatexText>}</span></p>
                </div>
                <div className="mt-5" style={{display: equation ? "block" : "none"}}>
                    <SolveButton onClick={() => {
                            const payload = {
                                dividend: extractPoly(poly1),
                                divisor: extractPoly(poly2),
                                p: parseInt(p)
                            };
                            fetch(`${BACKEND_URL}/poly_modulo`,
                                {
                                    method: "POST",
