    ...
    return (
        <div>
            <div className="flex flex-col items-center">
                <h1 className="text-4xl">Fermat Primary Test</h1>
                <p className="text-lg mt-4">
                    Determine whether <span>{<LatexText expression="a"></LatexText>}</span> is a witness of <span>{<LatexText expression="n"></LatexText>}</span> under Fermat Primary Test
                ...
                <div className="mt-5" style={{display: equation ? "block" : "none"}}>
                    <SolveButton onClick={() => {
                            fetch(`${BACKEND_URL}/primary?test=fermat&a=${a}&n=${n}`)
                            .then(async (res) => {
                            if (!res.ok) {
